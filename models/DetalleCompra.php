...
$this->id_medida = $id_medida;
    ...
    { 
        try { 
            ...
            for ($i = 0; $i < count($this->id_producto); $i++) {
                ...
                // Registrar la venta 
                $stmt = $this->conn->prepare("INSERT INTO compra (id_compra, id_producto, rif_proveedor, cantidad_compra, fecha, pago,monto) VALUES (:id_venta, :id_producto, :id_cliente, :cantidad, :fech_emision, :id_modalidad_pago, :monto)"); 
                ...
                $stmt = $this->conn->prepare("INSERT INTO detalle_compra_proveedor (id_detalleCompraProveedor, id_facturaProveedor, id_producto, cantidad_compra) VALUES (:id_detalleproducto, :id_venta, :id_producto, :cantidad)"); 
                $stmt->bindParam(':id_venta', $this->id_venta); 
                $stmt->bindParam(':id_producto', $producto_id); 
                $stmt->bindParam(':cantidad', $cantidad); 
            ...
            $nueva_cantidad = $producto['cantidad'] + $cantidad; // Cambia a suma
            ...
            if($m==$n)
            {
                $stmt = $this->conn->prepare("INSERT INTO cuenta_por_pagar (id_cuentaPagar, id_compra, fecha_cuentaPagar, monto_cuentaPagar) VALUES (:id_cuentaCobrar, :id_venta, :fecha_cuentaCobrar, :monto_cuentaCobrar)"); 
                $stmt->bindParam(':id_venta', $this->id_venta); 
    ...
    function Actualizar_Compra() {
        try {
            ...
            $querySelect = "SELECT monto_cuentaPagar FROM cuenta_por_pagar WHERE id_cuentaPagar = :id_venta";
            ...
            if ($stmtSelect->rowCount() > 0) {
                ...
                $queryUpdate = "UPDATE cuenta_por_pagar SET monto_cuentaPagar = :monto, fecha_cuentaPagar = :fech_emision WHERE id_cuentaPagar = :id_venta";
     ...
     function Eliminar_Compra($id_venta) {
        $query = "DELETE FROM compra WHERE id_compra = :id_venta";
        $stmt = $this->conn->prepare($query);
