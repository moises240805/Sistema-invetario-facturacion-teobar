...
<b><div style="color: black; font-size:1.3rem;" id="precioDolar">Cargando...</div></b>
        ...
        <main class="main">
    <header>
        <form class="formulario" action="crud_admin.php?action=actualizar" method="post" name="form">
            <h1 class="titulo_form">Modificar Usuario</h1>
            <?php if (!empty($message)): ?>
        <p class="alert alert-<?php echo ($message == "USUARIO ACTUALIZADO CORRECTAMENTE") ? 'success' : 'danger'; ?>">
            <?php echo $message; ?>
        </p>
    <?php endif; ?>
        <?php
        ...
        ?><br>
            <input class="entrada" type="hidden" name="id" value=<?php echo $admin['ID']; ?> required>
            </label></b>
            <b><label for="usuario">Usuario
            <input class="entrada" type="text" name="usuario" value=<?php echo $admin['usuario']; ?> required>
            </label></b>
            <b><label for="pw">Password
            <input class="entrada" type="password" name="pw" value=<?php echo $admin['pw']; ?> required>
            </label></b>
            <b><label for="rol">Rol
            <input class="entrada" type="text" name="rol" value=<?php echo $admin['rol']; ?> required>
            <br>
