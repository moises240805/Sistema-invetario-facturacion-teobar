...
<div id="root">
    ...
    <main class="main">
    <a class="hero__agg" href="crud_cliente.php?action=formulario">Agregar Cliente +</a>
    <h2 class="titulo">Gestionar Clientes</h2>
    <?php if (!empty($message)): ?>
        <p class="alert alert-<?php echo ($message == "CLIENTE ELIMINADO CORRECTAMENTE") ? 'success' : 'danger'; ?>">
            <?php echo $message; ?>
        </p>
    <?php endif; ?>
    <?php if (!empty($message3)): ?>
        <p class="alert alert-<?php echo ($message3 == "CLIENTE AGREGADO CORRECTAMENTE") ? 'success' : 'danger'; ?>">
            <?php echo $message3; ?>
        </p>
    <?php endif; ?>
    <?php if (!empty($message2)): ?>
        <p class="alert alert-<?php echo ($message2 == "CLIENTE ACTUALIZADO CORRECTAMENTE") ? 'success' : 'danger'; ?>">
            <?php echo $message2; ?>
        </p>
    <?php endif; ?>
    <div class="div">
    <table border="2px" class="table">
      <thead>
        <tr>
          <th style="background-color: rgba(0,0,0,.2);">CI</th>
          <th style="background-color: rgba(0,0,0,.2);">NOMBRE</th>
          <th style="background-color: rgba(0,0,0,.2);">TLF</th>
          <th style="background-color: rgba(0,0,0,.2);">EMAIL</th>
          <th style="background-color: rgba(0,0,0,.2);">DIRECCION</th>
        ...
        <tr>
          ...
          ?>
          <tr>
            <th><?php echo $cliente['tipo_id'] . $cliente['id_cliente']; ?></th>
            <td><?php echo $cliente['nombre_cliente']; ?></td>
            <td><?php echo $cliente['tlf']; ?></td>
            <td><?php echo $cliente['email']; ?></td>
            <td><?php echo $cliente['direccion']; ?></td>
            <td><a href="crud_cliente.php?action=mid_form&id_cliente=<?php echo $cliente['id_cliente']; ?>"><img title="Modificar" src="views/img/edit.png" width="40px" height="40px"></a>
            <a onclick="return eliminar()" href="crud_cliente.php?action=eliminar&ID=<?php echo $cliente['id_cliente']; ?>" class="btn"><img title="Eliminar"  src="views/img/delet.png" width="40px" height="40px"></a></td>
          </tr>
          <?php endforeach; ?>
        </tr>
      </thead>
